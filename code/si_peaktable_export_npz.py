...
from si_peaktable_stage2_dataset import SIPeakTableStage2Dataset
...
def main():
    ...
    ap.add_argument("--tmp_dir", default="tmp_export_si_peaktable")
    ...
    max_h1_peaks = int(ckpt.get("max_h1_peaks", 32))
    max_c13_peaks = int(ckpt.get("max_c13_peaks", 32))
    max_hsqc_peaks = int(ckpt.get("max_hsqc_peaks", 64))

    emb_dim = int(ckpt.get("emb_dim", 128))
    hidden = int(ckpt.get("hidden", 256))
    aux_hidden = int(ckpt.get("aux_hidden", 64))
    ...
    ds = SIPeakTableStage2Dataset(
        ...
        use_c13=use_c13,
        use_hsqc=use_hsqc,
        max_h1_peaks=max_h1_peaks,
        max_c13_peaks=max_c13_peaks,
        max_hsqc_peaks=max_hsqc_peaks,
    )
    ...
    model = SIPeakTableStage2Model(
        ...
        h1_feat_dim=ds.H1_FEAT_DIM,
        c13_feat_dim=ds.C13_FEAT_DIM,
        hsqc_feat_dim=ds.HSQC_FEAT_DIM,
        emb_dim=emb_dim,
        hidden=hidden,
    ...
    for batch in tqdm(dl, desc="Export"):
        ir, h1_x, h1_m, c13_x, c13_m, hsqc_x, hsqc_m, frag_ids, mask, _y = batch
